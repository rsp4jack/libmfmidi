...
#include "mfconfig.hpp"
#include <array>
#include <cstdint>


namespace libmfmidi::MIDIUtils {
    using MIDIClockTime = unsigned long;
    ...
    /// \code{.cpp}
    /// int length = LUT_SCMSGLEN[status-0xF0]
    /// \endcode
    constexpr std::array<int, 16> LUT_SCMMSGLEN = {
    ...
    /// \brief Get the Expected System Message Length object
    /// \warning The return value may be \b negative .
    /// \param[in] status Message Status
    /// \return int
    inline constexpr int getExpectedSCMMessageLength(uint8_t status)
    {
        return LUT_SCMMSGLEN.at(status - 0xF0);
    }
